...
def main():
    ...
    if n_steps*n_envs < 2 or (n_steps*n_envs) % batch:
    ...
    if args.resume:
        ...
        model = PPO("MlpPolicy", env, device=device, seed=args.seed, learning_rate=3e-4,
                    n_steps=n_steps, batch_size=batch, n_epochs=training.get("n_epochs", 5),
                    gamma=.99, gae_lambda=.95, clip_range=.2, ent_coef=.005, tensorboard_log=str(run/"tensorboard"),
