...
class KennyEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.gain = self.np_random.uniform(*self.config.motor_gain_range, size=2) if randomize else np.ones(2)
        self.slip = self.np_random.uniform(*self.config.slip_range, size=2) if randomize else np.ones(2)
        self.accel_scale = self.np_random.uniform(*self.config.acceleration_scale_range) if randomize else 1.
        self.delay = int(self.np_random.integers(0, self.config.command_delay_max_steps+1)) if randomize else 0
    ...
    def _sensor_outage(self, name):
        """Whole invalid samples in consecutive acquisition cycles, not stale data."""
        if self.outages[name] == 0 and self.config.sensor_outage_probability > 0:
            if self.np_random.random() < self.config.sensor_outage_probability:
                low, high = self.config.sensor_outage_steps
                self.outages[name] = int(self.np_random.integers(low, high+1))
        active = self.outages[name] > 0
        if active:
            self.outages[name] -= 1
        return active
